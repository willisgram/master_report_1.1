...
\textit{Step 1}. For the set of gameweeks in the sub-horizon:  generate forecasts of players' expected points, calculate empirical variance and correlation. 
\newline
...
\begin{algorithmic}
\State{$t$ = 0} 
\While{($t = t+1$) $\leq$ $\mathcal{|T|}$}
    \State{Generate input data: expected points, variance and correlation for $TP_{t}^{C}$ and $TP_{t}^{F}$}
    \If{a gameweek in $TP_{t}^{F}$ is \textit{double} or \textit{blank}}
        \State{Implement gamechip strategy}
    \ElsIf{a gameweek in $TP_{t}^{F}$ according to Wildcard strategy}
        \State{Implement Wildcard strategy}
    \EndIf
    \State{Solve the mathematical model for the problem defined by $TP^{C}_{t}$ and $TP^{F}_{t}$}
    \State{Fix variables $x_{pt}$, $v_{t}$ and $q_{t}$ in central period $TP^{C}_{t}$}
\EndWhile
\end{algorithmic}
