...
The solution approach suggested by \cite{Bonomo} can be used for the English Fantasy Premier League as well as for the Argentinian Fantasy League. In general, \cite{Bonomo} use the average points performance for the three last gameweeks to forecast the performance in the next gameweek. As mentioned in Section \ref{Forecasting_of_player_performance}, the forecast is further accounted weighting it on four factors: team opponent, home advantage, whether a player is on a good performance streak and whether a player is assumed to be in the starting line-up. 
